...
class TestBayesOpt(unittest.TestCase):
    ...
    def test_minimum_ucb(self):
        bo = BayesianOptimization(model, likelihood, target, search_space,"discrete_ucb",{"kappa":4})
        bo.optimal(10)
        y_actual = -1.48907
        x_actual = 0.96609
    ...
    def test_minimum_mes(self):
        bo = BayesianOptimization(model, likelihood, target, search_space,"discrete_mes",{"nK":1000})
        bo.optimal(10)
        y_actual = -1.48907
        x_actual = 0.96609
        self.assertLess(torch.norm(bo.x_star - x_actual), 1e-3)
    ...
    def test_minimum_pi(self):
        bo = BayesianOptimization(model, likelihood, target, search_space,"discrete_pi")
        bo.optimal(10)
        y_actual = -1.48907
        x_actual = 0.96609
        self.assertLess(torch.norm(bo.x_star - x_actual), 1e-3)
